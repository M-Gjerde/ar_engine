...
namespace vkglTF
{
    ...
    Mesh::Mesh(VulkanDevice *device, glm::mat4 matrix) {
        ...
        CHECK_RESULT(device->createBuffer(
                ...
                &uniformBlock));
        CHECK_RESULT(vkMapMemory(device->logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
        ...
        if (node.mesh > -1) {
            ...
            for (size_t j = 0; j < mesh.primitives.size(); j++) {
                ...
                // Vertices
                {
                    ...
                    posByteStride = posAccessor.ByteStride(posView) ? (posAccessor.ByteStride(posView) / sizeof(float)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC3);

                    if (primitive.attributes.find("NORMAL") != primitive.attributes.end()) {
                        const tinygltf::Accessor &normAccessor = model.accessors[primitive.attributes.find("NORMAL")->second];
                        const tinygltf::BufferView &normView = model.bufferViews[normAccessor.bufferView];
                        bufferNormals = reinterpret_cast<const float *>(&(model.buffers[normView.buffer].data[normAccessor.byteOffset + normView.byteOffset]));
                        normByteStride = normAccessor.ByteStride(normView) ? (normAccessor.ByteStride(normView) / sizeof(float)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC3);
                    }
                    ...
                    if (primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end()) {
                        ...
                        uv0ByteStride = uvAccessor.ByteStride(uvView) ? (uvAccessor.ByteStride(uvView) / sizeof(float)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC2);
                    }
                    if (primitive.attributes.find("TEXCOORD_1") != primitive.attributes.end()) {
                        const tinygltf::Accessor &uvAccessor = model.accessors[primitive.attributes.find("TEXCOORD_1")->second];
                        const tinygltf::BufferView &uvView = model.bufferViews[uvAccessor.bufferView];
                        bufferTexCoordSet1 = reinterpret_cast<const float *>(&(model.buffers[uvView.buffer].data[uvAccessor.byteOffset + uvView.byteOffset]));
                        uv1ByteStride = uvAccessor.ByteStride(uvView) ? (uvAccessor.ByteStride(uvView) / sizeof(float)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC2);
                    }
                    ...
                    if (primitive.attributes.find("JOINTS_0") != primitive.attributes.end()) {
                        ...
                        jointByteStride = jointAccessor.ByteStride(jointView) ? (jointAccessor.ByteStride(jointView) / tinygltf::GetComponentSizeInBytes(jointComponentType)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC4);
                    }
                    ...
                    if (primitive.attributes.find("WEIGHTS_0") != primitive.attributes.end()) {
                        ...
                        weightByteStride = weightAccessor.ByteStride(weightView) ? (weightAccessor.ByteStride(weightView) / sizeof(float)) : tinygltf::GetComponentSizeInBytes(TINYGLTF_TYPE_VEC4);
                    }
