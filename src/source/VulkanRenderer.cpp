...
int VulkanRenderer::init(GLFWwindow *newWindow) {
    ...
    try {
        ...
        uboViewProjection.projection = glm::perspective(glm::radians(45.0f),
                                                        ...
                                                        0.1f,
                                                        100.0f);
        uboViewProjection.view = glm::lookAt(glm::vec3(10.0f, 0.0f, 20.0f),
        ...
        std::vector<Vertex> meshVertices2 = {
                {{-0.25, 0.6,  0.0}, {0.0f, 0.0f, 1.0f}, {1.0f, 1.0f}},        // 0I love
...
*/
    ...
    result = vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &inputLayoutCreateInfo, nullptr,
                                         &inputSetLayout);
    if (result != VK_SUCCESS)
        throw std::runtime_error("Failed to create Descriptor Set Layout");
...
void VulkanRenderer::recordCommands(uint32_t currentImage) {
    ...
    for (size_t j = 0; j < modelList.size(); j++) {
        ...
        for (size_t k = 0; k < thisModel.getMeshCount(); k++) {
            ...
            VkBuffer vertexBuffers[] = {thisModel.getMesh(
                    k)->getVertexBuffer()};                                                       // Buffers to bind
    ...
    vkCmdBindDescriptorSets(commandBuffers[currentImage],
                            ...
                            0,
                            1,
                            &inputDescriptorSets[currentImage],
                            0,
                            nullptr);
...
void VulkanRenderer::createColorBufferImage() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        colorBufferImageView[i] = createImageView(
                colorBufferImage[i],
                colorFormat,
                VK_IMAGE_ASPECT_COLOR_BIT);
    }
